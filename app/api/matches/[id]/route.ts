...
export async function GET(
  ...
  try {
    ...
    // ✅ إذا كانت المباراة ملغية، نرجع 404 برضو
    if (match.status === 'cancelled') {
      return NextResponse.json(
        { success: false, error: 'المباراة ملغية' },
        { status: 404 }
      )
    }
    ...
    const responseMatch = {
      ...
      pendingRequests: (match.pendingRequests || []).map((r: any) => ({
        ...r,
...
export async function DELETE(
  ...
  try {
    ...
    // ✅ 2. إذا كان الحجز مرتبط بمباراة، نلغيها أيضاً
    if (booking.matchId) {
      ...
      // ✅ 3. إشعار كل اللاعبين اللي كانوا في المباراة
      const match = await db.collection('matches').findOne({
        _id: new ObjectId(booking.matchId)
      })
